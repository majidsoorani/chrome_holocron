...
5. **Enable Passwordless Sudo (Required for macOS)**
...
### OpenWrt Proxy Automation

- **`auto_refresh_proxy.py`**: A continuously running Python daemon designed to test and automatically switch your OpenWrt router's Passwall2 to the fastest available Cloudflare VLESS proxy. It is designed for 24/7 background operation.
  - **Features**: Includes zero-downtime smart updating (skipping Passwall restarts if the IP/Port hasn't changed), and robust fallback handling to local node (`127.0.0.1:1032`) or modem restart if the network drops completely.
  - **Usage**: Run `python3 auto_refresh_proxy.py` to start the 30-minute interval loop, or `python3 auto_refresh_proxy.py --run-once` for a single spot-check.
